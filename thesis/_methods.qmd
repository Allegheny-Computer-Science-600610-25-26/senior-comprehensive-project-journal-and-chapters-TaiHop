...
Figure 4 shows the flowchart of data processing by the tool.  
*Figure 4 Caption: The flowchart of data processing used by DataSeekr.*
...
Figure 5 shows the HTML structure and data paths used for scraping.  
*Figure 5 Caption: HTML structure and data extraction paths from the PAC site.*
...
As we discussed before on how the csv transforms into a database, this section will handle any questions regarding that topic. To begin things we need to get a good look at the concept or map of the steps of the conversion process. Below is a figure of the map of converter shown in [Figure 6]:

![Csv to DB Concept](images/Converter_Workkflow.png)

Figure 6 shows the workflow of converting CSV data into a database.  
*Figure 6 Caption: CSV to database conversion workflow.*
...
Moving onto the actual search part of the entire system. As shown in [Figure 7], this isn't directly connected to the scraping portion of my tool. This portion of the tool is mainly dependent on the database. When ```Seekr.py``` is run, it starts the Streamlit UI and at the same time this is connected to the database stored within the ```src``` folder. The figure below shows a more defined map of search functionality:

![Seekr map](images/seekr-map.png)

Figure 7 shows the workflow of search functionality.  
*Figure 7 Caption: Search and UI workflow using Streamlit.*
