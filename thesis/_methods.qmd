...
Figure 1 shows the flowchart of data processing by the tool.  
*Figure 1 Caption: The flowchart of data processing used by DataSeekr.*

The versions for the dependencies inside DataSeekr are as follows and summarized in [Table 1]:

**Table 1: Libraries and versions used in DataSeekr**
...
As we discussed before on how the csv transforms into a database, this section will handle any questions regarding that topic. To begin things we need to get a good look at the concept or map of the steps of the conversion process. Below is a figure of the map of converter shown in [Figure 3]:

![Csv to DB Concept](images/Converter_Workkflow.png)

Figure 3 shows the workflow of converting CSV data into a database.  
*Figure 3 Caption: CSV to database conversion workflow.*
...
Moving onto the actual search part of the entire system. As shown in [Figure 4], this isn't directly connected to the scraping portion of my tool. This portion of the tool is mainly dependent on the database. When ```Seekr.py``` is run, it starts the Streamlit UI and at the same time this is connected to the database stored within the ```src``` folder. The figure below shows a more defined map of search functionality:

![Seekr map](images/seekr-map.png)

Figure 4 shows the workflow of search functionality.  
*Figure 4 Caption: Search and UI workflow using Streamlit.*

Breaking down what happens inside the seekr function as partially mentioned above, streamlit is the topic. For my web face type output I am using streamlit due to many factors. Streamlit allows a full interactive web interface to be built entirely in Python, without needing separate frontend technologies such as HTML, CSS, or JavaScript. This significantly reduces development complexity while still producing a clean and professional interface.

The interface itself is built using simple components allowing an ease of setup. ```The st.text_input()``` captures the player’s name, while ```st.selectbox()``` provides a dropdown menu for selecting the statistical category (Hitting, Pitching, or Fielding). These serve as the input layer of the system. A conditional statement ensures that the search function only executes when both inputs are provided.

For the output, Streamlit directly renders the results returned by the database. The ```st.dataframe()``` function displays the player’s statistics in an interactive table format, allowing sorting and scrolling. Visualizations generated with matplotlib are embedded seamlessly using ```st.pyplot()```. If no results are found, ```st.warning()``` provides immediate user feedback.

The reasons for using matplotlib for the graphics are as follows:
	
- Seamless Integration with Pandas
- Strong Customization Control
- Handles Multiple Plots Easily
- Lightweight and efficient 
- Works well with streamlit 

Additionally, these plots help visualize trends and comparisons in player performance, making the data easier to interpret beyond raw numbers.

Overall, Streamlit functions as the presentation and interaction layer of the Seekr system. It bridges the gap between the user and the database by managing input collection, script execution, state handling, and output visualization.
